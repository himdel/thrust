...
/*==========================================================================*/
typedef struct {
  byte *Vstart, *Vcurrent;              /* ptr's into a playing sample  */
...
#else
  mix->Vclippedbuf = (byte *)calloc( sizeof(char), size);
#endif
...
Snd_loadRawSample(const char *file, Sample *sample, int loop)
{
  ...
  sample->data = (byte *)malloc(sample->len);
